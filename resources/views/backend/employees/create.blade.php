...
@section('content')
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <div class="card mb-4">
                <div class="card-body">
                    <h2 class="card-title h3 mb-2">新增員工</h2>
                    <p class="card-text text-muted">輸入員工基本資料，建立後即可透過其他模組進行進一步設定。</p>
                </div>
            </div>

            <div class="card shadow-sm">
                <div class="card-body">
                    <form action="{{ route('backend.employees.store') }}" method="POST" class="row g-3">
                        @csrf
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="employee_no" id="employee_no" value="{{ old('employee_no') }}" class="form-control" required>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="salary_grade" id="salary_grade" value="{{ old('salary_grade') }}" class="form-control">
                        </div>

                        <div class="col-md-4">
                            <label for="labor_grade" class="form-label">勞工等級</label>
                            <input type="text" name="labor_grade" id="labor_grade" value="{{ old('labor_grade') }}" class="form-control">
                        </div>

                        <div class="col-md-4">
                            <label class="form-label">身份屬性</label>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_indigenous" id="is_indigenous" value="1" {{ old('is_indigenous') ? 'checked' : '' }}>
                                <label class="form-check-label" for="is_indigenous">原住民</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_disabled" id="is_disabled" value="1" {{ old('is_disabled') ? 'checked' : '' }}>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" name="hired_at" id="hired_at" value="{{ old('hired_at') }}" class="form-control">
                        </div>

                        <div class="col-md-4">
                            <label for="terminated_at" class="form-label">離職日</label>
                            <input type="date" name="terminated_at" id="terminated_at" value="{{ old('terminated_at') }}" class="form-control">
                        </div>

                        <div class="col-12 d-flex justify-content-end gap-2">
                            <a href="{{ route('backend.employees.index') }}" class="btn btn-outline-secondary">取消</a>
                            <button type="submit" class="btn btn-primary">建立員工</button>
                        </div>
                    </form>
                </div>
            </div>
